...
Feature: Create Customer Order - Reusable feature
  ...
  Scenario: Test Order Creation - Create and verify order placement
    ...
    * print karate.pretty(requestPayload)
    When method POST
    * print 'headers:', karate.prevRequest.headers
    * print 'URL:', baseUrl+'/customerorder/v2/orders/'+getOrderId.orderId+'/product?channel'
    Then status 204
    * print karate.pretty(response)
    And match response $ == ''
...
#    * print 'Test ID : XYZ'
#    * print 'TestCase : Remove Product from the Basket | Positive'
#    * header traceId = traceId
#    * def requestPayload = read('classpath:com/cos/e2e/karate/template/removeProduct.json')
#    Given path '/customerorder/v2/orders/',getOrderId.orderId,'/product'
#    And param channel = 'online'
#    And request requestPayload
#    * print getOrderId.orderId
...
#    When method POST
#    * print 'headers:', karate.prevRequest.headers
#    * print 'URL:', '/customerorder/v2/orders/'+getOrderId.orderId+'/product'
#    Then status 204
#    * print karate.pretty(response)
#    And match response $ == ''
